...
(function (factory) {
    ...
    var range = (function () {
        ...
        var WrappedRange = function (sc, so, ec, eo) {
            ...
            this.wrapBodyInlineWithPara = function () {
                ...
                {
                    var scLength = sc.childNodes.length;
                    topAncestor = sc.childNodes[scLength > 0 ? scLength - 1 : 0];
                    //topAncestor = sc.childNodes[0];
                }
            ...
            create: function (sc, so, ec, eo) {
                if (!arguments.length) { // from Browser Selection
                    if (agent.isW3CRangeSupport) {
                        ...
                        if (selection.rangeCount === 0)
                        {
                            sc = document.getElementsByClassName("note-editable")[0];
                            so = 0;
                            ec = document.getElementsByClassName("note-editable")[0];
                            eo = 0;
                        ...
                        else
                        {
                            ...
                            if ((selection.getRangeAt(0).startContainer.offsetParent == undefined || selection.getRangeAt(0).startContainer.offsetParent.getAttribute('class') != 'note-editor') && (selection.getRangeAt(0).startContainer['nodeName'] != "#text" && selection.getRangeAt(0).startContainer['nodeName'] != 'P'))
                            {
                                sc = document.getElementsByClassName("note-editable")[0];
    ...
    var Editor = function () {
        ...
        this.insertImage = function ($editable, sUrl, filename)
        {
            ...
            async.createImage(sUrl, filename).then(function ($image) {
                ...
                //Gary Update for judge is there any paragraph tag in front of image tag 2015/10/19
                if (rangeCreate.ec['nodeName'] != "P" && rangeCreate.sc['nodeName'] != "P")
                {
                    //beforeCommand($editable);
                    typing.insertParagraph($editable);
                    //afterCommand($editable);
                    rangeCreate = range.create();
                }
                rangeCreate.insertNode($image[0]);
        ...
        this.fontSize = function ($editable, value) {
            ...
            if (agent.isFF) {
                // firefox: <font size="3"> to <span style='font-size={value}pt;'>, buggy
                $editable.find('font[size=3]').removeAttr('size').css('font-size', value + 'pt');
            }
            else
            {
                if ($editable.find('span').length > 0)
                {
                    // chrome: <span style="font-size: medium"> to <span style='font-size={value}pt;'>
                ...
                {
                    //Still Buggy Update Latter By Gary 2015/03/24
                    //$(document.getElementsByClassName('note-editable')).attr('class', 'note-editable font' + value);
                }

            }
            toolbar.updateFontsize(value);
            afterCommand($editable);
        };
    ...
    var Button = function () {
         ...
         * update fontsize
         * @param value
         * Update By Gary 2015/03/24
         */
        this.updateFontsize = function (value) {
            //value = the fonsize that selected
            var fontsize = document.getElementsByClassName("note-current-fontsize")[0];
            fontsize.textContent = value;
            $(document.getElementsByClassName('note-fontsize')[0]).find('.dropdown-menu li a').each(function () {
                // always compare string to avoid creating another func.
                var isChecked = ($(this).data('value') + '') === (parseFloat(value) + '');
                //$(this).attr('class') == isChecked ? 'checked' : '';
                if(isChecked)
                {
        ...
        this.updateRecentColor = function (button, eventName, value) {
            ...
            //文字背景顏色 Update By Gary
            if (sKey == 'background-color')
            {
    ...
    var Toolbar = function () {
        ...
        /***
         * @param value = the fontsize that selected
         */
        this.updateFontsize = function (value) {
            button.updateFontsize(value);
        };
    ...
    var EventHandler = function () {
        ...
        /**
         * isert Images from input file
         * New One Created By Gary
         */
        var insertImageByInput = function (layoutinfo, files)
        {
        ...
        var handleDragAndDropEvent = function (layoutInfo, options) {
         ...
         /**
         * Select file change Events
         *
         * @param {Object} layoutInfo - layout Informations
         */
        var handleFileChangeEvent = function (layoutInfo) {
            var $editable = layoutInfo.editable;
            var $imageInput = layoutInfo.toolbar.find('.note-image-input');
            // Cloning imageInput to clear element.
            $imageInput.replaceWith($imageInput.clone()
              .on('change', function ()
              {
                  //Gary Update 2015/03/20
                  $(".note-editable").focus();
                  //console.log("focus");
                  insertImageByInput(layoutInfo, this.files);
              })
              .val('')
            );
        };
        ...
        this.attach = function (layoutInfo, options) {
            ...
            if (!options.airMode) {
                ...
                handleFileChangeEvent(layoutInfo);
            }
    ...
    var Renderer = function () {
        ...
        var tplButtonInfo = {
            ...
            image: function (lang) {
                ...
                var input = '<input type="file" accept="image/*" name="files" class="note-image-input">';
                return input + '<div class="fa fa-picture-o icon-picture"></div>';
            },
            link: function (lang) {
            ...
            fcolor: function (lang) {
                var colorButtonLabel = '<i class="fa fa-font icon-font" style="color:black;"></i>';
            ...
            bcolor: function (lang) {
                var colorButtonLabel = '<i class="b-icon" style="color:black;"></i>';
                var colorButton = tplButton(colorButtonLabel, {
                    className: 'note-recent-color note-back-color',
                    title: lang.color.background,
                    event: 'color',
                    value: '{"backColor":"white"}'
                });
        ...
        var tplPopovers = function (lang, options) {
            ...
            var tplImagePopover = function () {
                ...
                /**
                 * 取消圖片可置左置右之功能，會引響至主題詳細頁之顯示樣式
                 * Gary Update 2015/03/24
                 */
                //var leftButton = tplIconButton('fa fa-align-left', {
