SuccApp.controller('SubjectDetail', ['$scope', 'SubDetail', '$http', '$sce', 'SubHistory', 'ReplyHistory', '$filter', 'MyFavCT', 'MyBoxCT', 'FavFactory', 'IsInsFactory', 'SubjectEvent', '$parse', 'ReplyService', 'ColorFactory', 'SubjectProgress', 'SubjectProgressBtn', '$rootScope', 'DiscType', 'AuditClass', 'AdminService', 'CopySubjUrl', 'DelSubjFactory', 'getfav', 'getmybox', 'RightSide', 'DelReplyFactory', 'colorbox', '$compile', 'ReplyTo', function ($scope, SubDetail, $http, $sce, SubHistory, ReplyHistory, $filter, MyFavCT, MyBoxCT, FavFactory, IsInsFactory, SubjectEvent, $parse, ReplyService, ColorFactory, SubjectProgress, SubjectProgressBtn, $rootScope, DiscType, AuditClass, AdminService, CopySubjUrl, DelSubjFactory, getfav, getmybox, RightSide, DelReplyFactory, colorbox, $compile, ReplyTo) {
    // #region 參數
    ...
    var SubjectID = "";
    var UserName = "";
    //判斷主題詳細頁來源從何開啟 0:來自未讀訊息,1:來自我的信箱,2:來自我的收藏,3:來自討論組列表,4:來自以讀訊息列表,5:來自申請歸檔
    var Source = "";
    //取得主題後，將自己的收件人資料放入
    var meInReceipts = [];
    var d = new Date();
    ...
    // #endregion
    // #region 接收主題詳細頁打開事件
    SubDetail.onOpenSubDetail($scope, function (Item) {
        ...
        UserName = _UserName;
        // 指定給目前點擊的詳細頁物件
        $scope.CurrentDetail.SubjID = SubjectID;
        $scope.CurrentDetail.CompID = CompanyID;
        $scope.CurrentDetail.UserID = UserID;
        $scope.CurrentDetail.DiscID = DiscID;
        $scope.CurrentDetail.UserName = UserName;

        RightSide.InSubjReNewScope($scope, SubjectID);
        $scope.CurrentSubjID = SubjectID;
        //做為收件人為自己時，刷新的假閱讀時間
        var FakeReadTime = Math.round(new Date().getTime() / 1000.0);
        // #region 呼叫Factory拿取主題詳細頁資料
        SubDetail.GetSubjDetail({ SubjectID: subID, UserID: UserID })
        .then(function (result) {
            ...
            //指定標題名稱
            $scope.$parent.subjectTitle = data.Title;
            //轉換主題詳細頁 內容html 
            //內簽圖格式修正
            ...
            if (typeof meInReceipts[0] !== 'undefined') {
                ...
                if (meInReceipts[0].UserReadStatus === 0) {
                    ...
                    SubDetail.UpdateReadState(meInReceipts[0], subID, UserID, CompID, DiscID);
            ...
            // #endregion
            // #region 主題草稿開啟
            var ReEditNewSubjDraft = $scope.$parent.ReEditNewSubjDraft;
            //$scope.$parent.ReEditNewSubjDraft = {};
            if (ReEditNewSubjDraft['TypeID'] == 0 && ReEditNewSubjDraft['isEdit'] == 1) {
                //編輯主題
                DraftToEditSubj(ReEditNewSubjDraft);
            }
            // #endregion
            // #region 回覆草稿開啟
            var ReplyDraftData = $scope.$parent.ReEditReplyDraft;
            if (ReplyDraftData['TypeID'] == 1 && ReplyDraftData['isEdit'] == 0) {
                //新增回復
                replySubject();
            }
            else if (ReplyDraftData['TypeID'] == 1 && ReplyDraftData['isEdit'] == 1) {
                //編輯回復
                EditWithReply(ReplyDraftData['ReplyId']);
            }
            // #endregion
        });
        // #endregion
        // #region 呼叫Factory拿取主題詳細頁回覆資料
        SubDetail.GetSubjReply({ SubjectID: subID, UserID: UserID })
        .then(function (result) {
            var data = result.data;
            var isAdmin = $scope.$parent.IndexData.CurrentUser['isAdmin'];
    ...
    // #endregion
    // #region 接收到關閉詳細頁主題，清空Scope
    RightSide.onReNewSubjDetailScope($scope);
    // #endregion
    // #region 編輯主題 
    ...
    // #endregion
    // #region 草稿進入編輯主題的路線
    function DraftToEditSubj(EditSubjDraft) {
        //關閉紀錄
        $scope.ShowSubHis = false;
        //進入編輯主題狀態
        $scope.isEditSubj = true;
    ...
    // #endregion
    // #region 回覆主題 
    $scope.ReplySubject = replySubject;
    ...
    // #endregion
    // #region 主題狀態  0,1,2,3 申請歸檔 進行中 完成 退回
    ...
    // #endregion
    // #region 編輯 By 回覆
    $scope.EditByReply = EditWithReply;
    function EditWithReply(para) {
        $scope.reply2lv = '';
        $scope.isEditSubj = false;
        $scope.isReply = false;
    ...
    // #endregion
    // #region 行政區審核行為還原
    //$scope.RestoreCheck = function (subjId, flowItem, progress) {
    //    // 是進行中是關主且審核到這一關可以還原審核
    //    if (flowItem.AuditID === UserID && flowItem.Serise === $scope.lastFlowSerise && (progress === SubjectProgress.InProgress || progress === SubjectProgress.Reject)) {
    //        if (subjId !== null) {
    //            // 塞進viewmodel
    //            // 傳給serivce
    //            var promise = AdminService.RestoreAuditSubmit(new UpdateAudit({
    //                SubjID: subjId || null,
    //                State: null,
    //                Serise: flowItem.Serise || null,
    //                UserID: UserID || null
    ...
    // #endregion
    // #region 行政區重新審核圈圈樣式判斷
    $scope.ReCheckClass = function (flowItem) {
        if (flowItem.Serise <= $scope.lastFlowSerise) {
            // 可以按
            return AuditClass.UnCheck;
        } else {
            // 不能按
            return AuditClass.UnCheckGray;
        }
    };
    // #endregion
    ...
    $scope.export = function () {
        ...
        //var WebSiteURL = "http://localhost:2042";
        ...
        //ToolBar
        var ToolBarEntity = $("#toolbar").children("div");
        //MyBox
        ...
        if ($scope.data.SubjModifiedById != '') {
            var Modify = $(".detail_infobar_hr").children("a").text();
            var ModifyInfo = $(".detail_infobar_hr").children("label").text();
