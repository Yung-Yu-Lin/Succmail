SuccApp.directive("screening", ['$http', '$rootScope', 'Filter_Create_Start', 'Filter_Create_End', 'Filter_Modifyed_Start', 'Filter_Modifyed_End', '$filter', 'GetListFinish', function ($http, $rootScope, Filter_Create_Start, Filter_Create_End, Filter_Modifyed_Start, Filter_Modifyed_End, $filter, GetListFinish) {
    ...
    //初始化取得公司成員列表(收件人)
    function InitCompanyUser(scope,compid)
    {
        $http({
            method: 'get',
            url: '/Subject/GetCompanyUserList/?CompanyID=' + compid
        })
    ...
    {
        return data.CreatedOn < StartDate && data.CreatedOn >= EndDate;
    }
    ...
    //實作最後回覆時期排序，傳送Function給FilterPara
    function realize_Modifyed_Filter(data, StartDate, EndDate)
    {
        return data.PlanCloseOn < StartDate && data.PlanCloseOn >= EndDate;
    }

    //實作最後回覆日期，自行選擇
    function realize_Modifyed_Date(scope, Modifyed_Date_Start, Modifyed_Date_End) {
        scope.EndDate = {};
        if (Modifyed_Date_Start == "" && Modifyed_Date_End == "") {
            ...
            scope.FilterModify = function (data) {
                return data.PlanCloseOn > (Date.parse(Modifyed_Date_Start) / 1000);
            }
            //實作日期Tag
            ...
            scope.FilterModify = function (data) {
                return data.PlanCloseOn <= (Date.parse(Modifyed_Date_End) / 1000);
            }
            //實作日期Tag
            ...
            scope.FilterModify = function (data) {
                return data.PlanCloseOn > (Date.parse(Modifyed_Date_Start) / 1000) && data.PlanCloseOn <= (Date.parse(Modifyed_Date_End) / 1000);
            }
            //實作日期Tag
    ...
    function GroupByUserData(scope, ListData, type)
    {
        //type:0 非討論組列表 type:1 討論組列表
        //初始化建立人及最後回覆人為空陣列
        var CreatorData = [];
        var ModifierData = [];
        //重組SubjectList
        ...
        if (type == 0)
        {
            ...
            {
                for (var j = 0; j < ListData[i].SubjectList.length; j++)
                {
                    SubjectArray.push(ListData[i].SubjectList[j]);
                }
            }
        }
        else if (type == 1)
        {
            if (ListData.SubjectList == null)
            {
                return;
            }
            for(var i = 0; i < ListData.SubjectList.length;i++)
            {
                SubjectArray.push(ListData.SubjectList[i]);
            }
        }

        //進行Distinct
        var CreatorArray = $filter('distinct')(SubjectArray, "CreatedBy");
        var ModifierArray = $filter('distinct')(SubjectArray, "ModifiedBy");
        //建立者
    ...
    return {
        ...
        template: "<button class='condition-btn-filter btn btn-sm' ng-click='OpenFilter()'>" +
                  ...
                  "<ul class='dropdown-menu drop-filter' style='display:block;' ng-show='FilterVisible'>" +
                       "<li class='dropdown-submenu'>" +
                           "<a class='dropdown-submenu-a a_type'>" + $filter('translate')('Urgency') + "</a>" +
                           "<ul class='dropdown-menu'>" +
                               "<li><a ng-click='DoEmergencyLelve(" + '"3"' + ")' tabindex='-1' class='a_type'>" +
                                   $filter('translate')('Urgent') +
                               "</a></li>" +
                               "<li><a ng-click='DoEmergencyLelve(" + '"2"' + ")' tabindex='-1' class='a_type'>" +
                                   $filter('translate')('Pressing') +
                               "</a></li>" +
                               "<li><a ng-click='DoEmergencyLelve(" + '"1"' + ")' tabindex='-1' class='a_type'>" +
                                   $filter('translate')('Important') +
                               "</a></li>" +
                               "<li><a ng-click='DoEmergencyLelve(" + '"0"' + ")' tabindex='-1' class='a_type'>" +
                                   $filter('translate')('Unmarked') +
                               "</a></li>" +
                           "</ul>" +
                       "</li>" +
                       "<li class='dropdown-submenu'>" +
                       ...
                       "<li><label>" + $filter('translate')('Date') + "</label><br></li>" +
                                ...
                                "<ul class='dropdown-menu' style='width:120px;top:-105px;' ng-controller='DatepickerDemoCtrl'>" +
                                    ...
                                    "<li class='FilterDatePicker'>" +
                                        "<p class='input-group input-group-sm start-date pull-left'>" +
                                            "<input type='text' class='form-control' datepicker-popup='{{format}}' ng-model='Create_Date_Start' is-open='StartCreateOpened' min-date='1990-01-01' max-date=''2015-06-22'' datepicker-options='dateOptions' date-disabled='disabled(date, mode)' ng-required='true' close-text='Close' show-button-bar='show_button' ng-keydown='SubjKeydown($event)' />" +
                                    ...
                                    "<li class='FilterDatePicker'>" +
                                        "<p class='input-group input-group-sm start-date pull-left'>" +
                                            "<input type='text' class='form-control' datepicker-popup='{{format}}' ng-model='Create_Date_End' is-open='LastCreateOpened' min-date='1990-01-01' max-date=''2015-06-22'' datepicker-options='dateOptions' date-disabled='disabled(date, mode)' ng-required='true' close-text='Close' show-button-bar='show_button' ng-keydown='SubjKeydown($event)' />" +
                                            "<span class='input-group-btn'>" +
                        ...
                        "<li class='dropdown-submenu'>" +
                            "<a class='a_type'>" +
                                $filter('translate')('End') + $filter('translate')('Date') +
                                "<ul class='dropdown-menu' style='width:120px;top:-132px;' ng-controller='DatepickerDemoCtrl'>" +
                                    ...
                                    "<li class='FilterDatePicker'>" +
                                        "<p class='input-group input-group-sm start-date pull-left'>" +
                                            "<input type='text' class='form-control' datepicker-popup='{{format}}' ng-model='Modifyed_Date_Start' is-open='StartModifyOpened' min-date='1990-01-01' max-date=''2015-06-22'' datepicker-options='dateOptions' date-disabled='disabled(date, mode)' ng-required='true' close-text='Close' show-button-bar='show_button' ng-keydown='SubjKeydown($event)' />" +
                                            "<span class='input-group-btn'>" +
                                    ...
                                    "<li class='FilterDatePicker'>" +
                                        "<p class='input-group input-group-sm start-date pull-left'>" +
                                            "<input type='text' class='form-control' datepicker-popup='{{format}}' ng-model='Modifyed_Date_End' is-open='EndModifyOpened' min-date='1990-01-01' max-date=''2015-06-22'' datepicker-options='dateOptions' date-disabled='disabled(date, mode)' ng-required='true' close-text='Close' show-button-bar='show_button' ng-keydown='SubjKeydown($event)' />" +
                                            "<span class='input-group-btn'>" +
        ...
        {
            //非完成區不可自行輸入日期
            scope.SubjKeydown = function ($event) {
                $event.preventDefault();
                return false;
            };
            //非完成區 顯示結束日期之取消按鈕
            scope.closedate = true;
            scope.dateitem = $filter('translate')('End') + $filter('translate')('Date');
            //初始化資料
            InitData(scope);
            //初始化各別討論組
            if (attrs.disctype == "general")
            {
                //取得收件人列表
                InitDiscUser(scope, attrs.discid);
                GetListFinish.onGetFinish(scope, function ()
                {
                    GroupByUserData(scope, scope.DiscListData,1);
                });
            }
            //初始化收藏夾
            else if (attrs.disctype == "fav")
            {
                InitCompanyUser(scope, attrs.compid);
                GetListFinish.onGetFinish(scope, function ()
                {
                    GroupByUserData(scope, scope.FavData,0);
                });
            }
            //初始化未讀訊息
            else if (attrs.disctype == "noread")
            {
                //初始化公司員工列表(收件人)
                InitCompanyUser(scope, attrs.compid);
                //確定未讀訊息已經有拿到資料
                ...
                {
                    var DiscArray = [];
                    for (var i = 0; i < scope.AllNordData.length; i++)
                    {
                        for(var j = 0;j<scope.AllNordData[i].Disc.length;j++)
                        {
                            DiscArray.push(scope.AllNordData[i].Disc[j]);
                        }
                    }
                    GroupByUserData(scope, DiscArray,0);
                });
            }
            //初始化我的信箱
            else if (attrs.disctype == "mybox")
            {
                //初始化公司員工列表(收件人)
                InitCompanyUser(scope, attrs.compid);
                //確定我的信箱已經有拿到資料
                ...
                {
                    //實作建立者及最後回覆者
                    GroupByUserData(scope, scope.MyBoxData,0);
                });
            }
            //初始化搜尋結果(主題)
            else if (attrs.disctype == "search")
            {
                //初始化公司員工列表(收件人)
                InitCompanyUser(scope, attrs.compid);
                //實作建立者及最後回覆者
                GroupByUserData(scope, scope.$parent.SearchData,0);
            }
            ...
            Filter_Create_Start.onFilterDate(scope, function (item) {
                realize_Create_Date(scope, item.StartDate, item.EndDate);
                CloseFilterArea(scope);
            });
            ...
            scope.EDateRange = function(para)
            {
                ...
                switch (para) {
                    case "1":
                        ...
                        scope.FilterModify = function (data) {
                            return realize_Modifyed_Filter(data, TodayString, OneWeekString);
                        }
                        break;
                    case "2":
                        //兩星期內
                        ...
                        scope.FilterModify = function (data) {
                            return realize_Modifyed_Filter(data, TodayString, TwoWeeksString);
                        }
                        break;
                    case "3":
                        //一個月內
                        ...
                        scope.FilterModify = function (data) {
                            return realize_Modifyed_Filter(data, TodayString, OneMonthString);
                        }
                        break;
            ...
            Filter_Modifyed_Start.onFilterDate(scope, function (item) {
                realize_Modifyed_Date(scope, item.StartDate, item.EndDate);
                CloseFilterArea(scope);
            });
