SuccApp.controller('AddNewPurchasCtrl', ['$scope', 'FileUploader', '$log', 'AttatchService', 'ImageService', 'filterFilter', '$filter', '$rootScope', 'AddNewPurchasService', 'DiscType', 'SubjectColor', '$timeout', 'SubjectEvent', 'Draft', '$interval', '$translate', 'EditorPara', 'AdminService', 'ReceiptsService', 'RightSide','$sce', function ($scope, FileUploader, $log, AttatchService, ImageService, filterFilter, $filter, $rootScope, AddNewPurchasService, DiscType, SubjectColor, $timeout, SubjectEvent, Draft, $interval, $translate, EditorPara, AdminService, ReceiptsService, RightSide,$sce)
{
    ...
    $scope.open = function ($event, $index) {
        ...
        $scope.purchases[$index].datepickerOpened = true;
    };
    ...
    $scope.save = function ()
    {
        ...
        var result = AdminService.ValidPurchase($scope.purchases);
        $scope.purchases = result.Model;
        ...
        $scope.NewPurchas = {
            ...
            Purchas: $filter('adminFilter')($scope.purchases, 'pName'),
            //行政區暫不增加主題標籤
            SubjTagID: null,
            SubjTagName: '',
            CusTagID: null,
        ...
        // 確定儲存
        AddNewPurchasService.save($scope.NewPurchas)
        .success(function (response) {
            if (response.IsSuccessful === true) {
                $scope.loadingText = $filter('translate')('CreateMsgOk');
                $scope.NewPurchas.SubjectID = response.DataObj.SubjectID;
                $scope.NewPurchas.CreatedOn = response.DataObj.CreatedOn;
                $scope.NewPurchas.ModifiedOn = response.DataObj.ModifiedOn;

                // 廣播新增完成由companyMessageCtrl監聽
                $rootScope.$broadcast(SubjectEvent.Inserted, $scope.NewPurchas);
